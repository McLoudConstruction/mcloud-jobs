            ...
            {show('job_counts_by_stage') && (
              <div className="card">
                ...
                {isMobile ? (
                  ...
                  // columnGap was 20px — with 9 stages + Total sharing the
                  // card's width, that left too little room per column for
                  // longer labels ("Estimate Delivered") at nowrap, so the
                  // text overflowed its own column and visually overlapped
                  // the next one. Tightening the gap gives each column more
                  // real width, and dropping nowrap lets a label that still
                  // doesn't fit wrap onto a second line instead of bleeding
                  // sideways into its neighbor.
                  <div style={{ display: 'grid', gridTemplateColumns: `repeat(${STAGE_ORDER.length}, minmax(0, 1fr)) auto`, columnGap: 8 }}>
                    {STAGE_ORDER.map(s => (
