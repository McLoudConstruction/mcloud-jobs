...
export default function IndividualProposalDocumentPage() {
  ...
  async function downloadDocument() {
    setDownloading(true);
    try {
      const filename = docFilename(`Estimate-${proposal.name}`, job.customer_name);
    ...
    <div>
      <div className="no-print doc-toolbar">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <ShareLinkButton kind="proposal" jobId={id} proposalId={proposal.id} customerName={job.customer_contact || job.customer_name} />
        </div>
      </div>
      ...
      <SendDocModal
        ...
        pdfFilename={docFilename(`Estimate-${proposal.name}`, job.customer_name)}
