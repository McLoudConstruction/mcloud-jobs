...
export default function EstimatePage() {
  ...
  const loadJob = useCallback(async () => {
    ...
    if (data) { setJob(data); setMargin(data.estimate_margin_percent != null ? String(data.estimate_margin_percent) : ''); }
  }, [id]);

  const loadActions = useCallback(async () => {
    const { data } = await supabase.from('job_scope_actions').select('*').eq('job_id', id).order('trade');
    if (data) setActions(data);
  }, [id]);

  const loadItems = useCallback(async () => {
    const { data } = await supabase.from('job_estimate_items').select('*').eq('job_id', id).order('created_at');
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`estimate-${id}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'job_estimate_items', filter: `job_id=eq.${id}` }, loadItems)
      .subscribe();
    return () => supabase.removeChannel(channel);
  }, [session, id, loadJob, loadActions, loadItems]);
  ...
  function saveMargin(value) {
    ...
    marginSaveTimer.current = setTimeout(() => {
      supabase.from('jobs').update({ estimate_margin_percent: value ? parseFloat(value) : null }).eq('id', id);
    }, 500);
  }
    ...
    <AppShell>
      <div className="container container-wide">
        <div className="top-actions">
          <div>
            <h2 style={{ margin: 0, color: 'var(--heading)' }}>Estimate — Job #{job.job_number}</h2>
            <div style={{ fontSize: 12, color: 'var(--ink-soft)' }}>{job.customer_name} · {job.project_address}</div>
          </div>
        ...
        <div style={{ fontSize: 11.5, color: 'var(--ink-soft)', marginBottom: 16, background: 'var(--panel)', border: '1px solid var(--line)', borderRadius: 6, padding: '10px 14px' }}>
          This is a standalone experimental tool — nothing here affects the job's real Contract Price or Financials until you decide to use it. Every quantity and price is yours to set; nothing is calculated or padded for you.
        </div>
        ...
        <div className="estimate-grid">
            ...
            <div className="card">
              ...
              {actions.length === 0 && (
                <div style={{ fontSize: 11.5, color: 'var(--ink-soft)', marginTop: 6 }}>
                  No exhaustive action list yet — build one on the job's Scope tab first, or just add line items manually below.
                </div>
              )}
          ...
          <div className="estimate-sidebar">
            <div className="card">
              <h3>Exhaustive Action List</h3>
              <div style={{ fontSize: 11, color: 'var(--ink-soft)', marginBottom: 10 }}>Reference only — what this job actually requires, by trade.</div>
              {Object.keys(grouped).length === 0 && <div className="empty-state">Nothing built yet on the job's Scope tab.</div>}
              {Object.entries(grouped).map(([trade, rows]) => (
                <div key={trade} style={{ marginBottom: 14 }}>
                  <div style={{ fontWeight: 700, fontSize: 11, letterSpacing: '0.04em', textTransform: 'uppercase', color: 'var(--gold)', marginBottom: 4 }}>{trade}</div>
                  {rows.map(a => (
                    <div key={a.id} style={{ fontSize: 12, padding: '4px 0', borderBottom: '1px solid var(--line)' }}>
