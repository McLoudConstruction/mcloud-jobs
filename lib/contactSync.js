...
async function syncContactToPeople({ linkColumn, linkId, extraFields, name, phone, email }) {
  ...
  // Best-effort background sync called from several forms without the
  // caller checking a return value — a silent failure here previously
  // meant the linked Person just never showed up, with nothing to explain
  // why.
  if (error) console.error('syncContactToPeople failed:', error.message);
}
...
export async function linkOrCreateCompanyByName(companyName, { defaultCompanyType = null } = {}) {
  ...
  const { data: created, error } = await supabase
    ...
    .select()
    .single();
  if (error) { console.error('linkOrCreateCompanyByName failed:', error.message); return null; }
...
// Find-or-create for the manual Sales Route builder: unlike
// findPropertyIdByName above, this one DOES create a property when
// nothing matches — a route stop picked from a real-world address
// lookup already has everything a property record needs (name +
// address), unlike a contact's free-text "property" field.
//
// Matches on street address first (same building even if the name was
// typed differently — "Oakwood Apts" vs "Oakwood Apartments"), falling
// back to name match when there's no address to compare (the user typed
// a name but never picked an autocomplete result). Returns the existing
// or newly-created property row, plus whether it was newly created.
export async function findOrCreatePropertyForRouteStop({ name, street, city, state, zip, lat, lng }) {
  const trimmedName = (name || '').trim();
  const trimmedStreet = (street || '').trim();
  if (!trimmedName) return { property: null, created: false };
  ...
  if (trimmedStreet) {
    const { data } = await supabase
      .from('properties')
      .select('*')
      .ilike('property_street', trimmedStreet)
      .limit(5);
    const candidates = data || [];
    existing = candidates.find(p => zip && (p.property_zip || '') === zip) || candidates[0] || null;
  }
