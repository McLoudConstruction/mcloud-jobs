...
// Unfiltered read of every queue item, 'syncing' included. getPendingQueue()
// deliberately hides 'syncing' items (so a second flush wave doesn't
// re-upload one that's mid-flight right now), but that same filter made a
// *stranded* item — one that got marked 'syncing' and then never reached
// 'pending', 'failed', or removed, because the tab died mid-upload —
// invisible everywhere: not retried, no error shown, not even visible to a
// direct "is anything stuck" check. This is for the two places that need to
// see the truth: the stale-'syncing' reclaim in syncQueue.js, and any
// diagnostic that wants to report a stranded item instead of silently
// missing it.
