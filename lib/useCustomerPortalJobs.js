...
import { flattenJobFinancials } from './jobFinancials';
...
export function useCustomerPortalJobs(session) {
  ...
  const loadJobs = useCallback(async () => {
    const { data } = await supabase.from('jobs').select('*, job_financials(contract_price, invoice_amount, invoice_status)').order('created_at', { ascending: false });
    if (data) {
    ...
    // Set regardless of whether data came back — a customer whose only
    // job was just closed lost now correctly gets an empty (not missing)
    // result from RLS, and the portal needs to tell "still loading" apart
    // from "confirmed, there's nothing here" to show the right screen.
    setJobsLoaded(true);
  }, []);
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'job_financials' }, loadJobs)
      .subscribe();
  ...
  return { jobs, jobsLoaded, selectedJobId, setSelectedJobId, job };
}
