...
// Saves the generated PDF to the user's device under the given file name.
// (Opening the blob in a new tab leaves the file named after a random blob
// id, and a new tab opened after an await is often killed by popup
// blockers — an anchor with a download name avoids both.)
export function downloadPdf(base64, filename) {
...
function calculateBreakOffsets(root) {
  ...
  // The footer is a couple lines of text — never worth stranding alone on
  // an otherwise-blank page. Fold its height into the unit before it so a
  // break is never evaluated right before the footer specifically; it just
  // rides along with whatever page the preceding content lands on.
  if (units.length > 1 && units[units.length - 1].el.classList.contains('doc-footer')) {
    const footer = units.pop();
    const prev = units[units.length - 1];
    units[units.length - 1] = { el: prev.el, height: prev.height + footer.height };
  }
