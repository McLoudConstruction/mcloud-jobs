...
// jobs "Lastname — Job #204" rather than leading with the address.
// Just the surname: takes the last whitespace-separated token of
// customer_name. customer_name is a company name on commercial jobs
...
// The sub portal's job heading: "Smith — Job #204". Callers must select
// customer_name, job_number, estimate_number and stage on the job they
// pass in.
export function subPortalJobHeading(job) {
  if (!job) return 'Job details unavailable';
  const last = customerLastName(job);
  const num = projectNumber(job);
  const label = projectNumberLabel(job);
  const numPart = num && num !== '—' ? `${label} #${num}` : '';
