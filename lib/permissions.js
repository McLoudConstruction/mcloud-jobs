...
export const ROLES = ['owner', 'general_manager', 'estimator', 'project_manager', 'bookkeeper'];
...
const ROUTE_RULES = [
  ...
  // Most specific first — RFPs are Owner/Estimator only, checked before
  // the broader /subcontractors rule below so it doesn't fall through.
  { prefix: '/subcontractors/rfps', roles: ['owner', 'estimator'] },
  { prefix: '/subcontractors', roles: ['owner', 'general_manager', 'estimator', 'project_manager'] },
  ...
  // Estimator needs the Individual Job page itself to send RFPs job by
  // job (Financials → RFPs section, role-gated further within the page)
  // — this covers /jobs, /jobs/[id], /jobs/calendar, /jobs/new, /jobs/lost.
  { prefix: '/jobs', roles: ['owner', 'general_manager', 'estimator', 'project_manager'] },
