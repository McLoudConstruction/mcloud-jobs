...
import { ensureDecodableImage } from './heicConvert';
...
function loadImage(src, crossOrigin) {
  return new Promise((resolve, reject) => {
    ...
    // The native <img> onerror handler fires with a plain Event, not an
    // Error — leaving it as the rejection reason surfaces as "Upload
    // failed: undefined" in the UI (no `.message` on an Event). Wrap it
    // in a real Error so the person sees an actual explanation.
