...
export function buildSubInviteEmail({ applyUrl, companyHint }) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 8px;">
      ...
      <a href="${applyUrl}" style="display: inline-block; background: ${BRAND_BROWN}; color: #ffffff; font-size: 14px; font-weight: 700; text-decoration: none; padding: 13px 26px; border-radius: 6px;">
...
export function buildReviewRequestEmail({ customerName, reviewUrl }) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 8px;">
      ...
      <a href="${reviewUrl}" style="display: inline-block; background: ${BRAND_BROWN}; color: #ffffff; font-size: 14px; font-weight: 700; text-decoration: none; padding: 13px 26px; border-radius: 6px;">
...
export function buildPortalInviteEmail({ customerName, actionLink }) {
  const firstName = (customerName || 'there').split(' ')[0];
  const subject = 'Your McLoud Construction project portal';
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 8px;">
      ...
      <p style="font-size: 14px; color: #4a4436; line-height: 1.6; margin: 0 0 26px;">
        Hi ${firstName}, click below to sign in to your McLoud Construction project portal — everything about your
        project (estimates, contracts, updates, and invoices) lives there in one place.
      </p>
      <a href="${actionLink}" style="display: inline-block; background: ${BRAND_BROWN}; color: #ffffff; font-size: 14px; font-weight: 700; text-decoration: none; padding: 13px 26px; border-radius: 6px;">
  ...
  const text = `You're Invited\n\nHi ${firstName}, sign in to your McLoud Construction project portal here:\n\n${actionLink}\n\nThis link is unique to you and expires soon — if it's stopped working, just ask us to resend it.\n\nMcLoud Construction`;
...
export function buildStaffInviteEmail({ fullName, roleLabel, actionLink }) {
  ...
  const html = `
    <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; max-width: 480px; margin: 0 auto; padding: 8px;">
      ...
      <a href="${actionLink}" style="display: inline-block; background: ${BRAND_BROWN}; color: #ffffff; font-size: 14px; font-weight: 700; text-decoration: none; padding: 13px 26px; border-radius: 6px; margin-top: 16px;">
        Set Up My Account
      </a>
      <p style="font-size: 12px; color: #8a8471; line-height: 1.6; margin: 26px 0 4px;">
