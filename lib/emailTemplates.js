...
export function buildScheduleReminderEmail({ customerName, projectAddress, scheduledStartDate, daysOut }) {
  ...
  const text = `${heading}\n\nHi ${firstName}, just a reminder that work on your project${projectAddress ? ` at ${projectAddress}` : ''} is scheduled to begin ${timeframe}, on ${dateLabel}. Sign in at jobs.mcloudconstruction.com/portal to check your project details, or reach out if anything's changed.\n\nMcLoud Construction`;
