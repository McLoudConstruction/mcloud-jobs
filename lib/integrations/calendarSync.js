...
// Fallback only — the real value is Settings → Automation → Timezone
// (app_settings.timezone, migration 112), read fresh per sync below. Used
// if that column is somehow still null (a fresh install before running
// migration 112). schedule_events.event_date/event_time are naive
// wall-clock values entered through a browser date/time picker (no
...
// here parses as 15:15 UTC, not 15:15 local — a straight "3:15pm shows up
// as 10:15am" 5-hour bug (for Central time; the exact shift depends on
// the configured zone). Job/bid-walk pushes don't need this: their source
// columns (scheduled_start_date is date-only/all-day; bid_walk_
...
export async function syncConnection(connection) {
  ...
  const { data: appSettings } = await admin.from('app_settings').select('timezone').eq('id', 1).maybeSingle();
  const businessTimezone = appSettings?.timezone || DEFAULT_TIMEZONE;
  ...
  if (timedPusher) {
    ...
    for (const ev of scheduleEvents || []) {
      ...
      try {
        let event;
        if (ev.event_time) {
          const start = zonedTimeToUTC(ev.event_date, ev.event_time, businessTimezone);
