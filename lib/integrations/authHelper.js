...
// Same shape as requireOwner, but for routes any active staff member
// (any role) should be able to hit — not just an owner managing Settings.
// Used by the google-maps-key route: every staff member using Properties
// or Sales needs the key to load address autocomplete, not just the
// owner who pasted it into Settings.
export async function requireStaff(request) {
  try {
    ...
    const admin = getAdminClient();
    const { data: staff } = await admin.from('staff_users').select('id, status').eq('id', user.id).single();
    if (!staff || staff.status !== 'active') {
      return { error: 'Not an active staff account.', status: 403 };
    }
    return { staffId: user.id };
  } catch (err) {
    return { error: err.message, status: 500 };
