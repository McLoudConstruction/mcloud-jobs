...
// forever.
async function fetchTimelinePage(resolution, wantCount, lat, lng, startSec, apiKey, maxPages = 5) {
  const records = [];
  let start = startSec;
...
export async function getForecast(lat, lng, { force = false } = {}) {
  ...
  const [hourlyRecords, dailyRecords] = await Promise.all([
    fetchTimelinePage('1h', 48, lat, lng, nowSec, apiKey),
    fetchTimelinePage('1day', 8, lat, lng, nowSec, apiKey),
  ]);
