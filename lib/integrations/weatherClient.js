...
// Fetches one OpenWeather 4.0 timeline resolution, following its `next`
// pagination cursor until either `wantCount` records are collected or
// the API stops offering more. `cnt` is only a request, not a
// guarantee: OpenWeather was confirmed (via a debug fetch) to hard-cap
// each page at 20 records regardless of what `cnt` asks for, handing
// back a `next` URL for the rest. maxPages caps this at a handful of
// requests so a misbehaving or endless `next` can't loop forever.
async function fetchTimelinePage(resolution, wantCount, lat, lng, startSec, apiKey, maxPages = 5, debugLog = null) {
  const records = [];
  let nextUrl = `https://api.openweathermap.org/data/4.0/onecall/timeline/${resolution}?lat=${lat}&lon=${lng}&cnt=${wantCount}&start=${startSec}&units=imperial&appid=${apiKey}`;

  for (let page = 0; page < maxPages && records.length < wantCount && nextUrl; page++) {
    const res = await fetch(nextUrl);
    if (!res.ok) throw new Error(`Forecast lookup failed: ${await res.text()}`);
    const raw = await res.json();
    const pageRecords = raw.data || raw.hourly || raw.daily || [];
    if (debugLog) debugLog.push({ resolution, page, recordCount: pageRecords.length, next: raw.next ?? null });
    if (pageRecords.length === 0) break;
    records.push(...pageRecords);

    if (!raw.next) { nextUrl = null; break; }
    // `next` has been observed as a full URL in practice; handling a
    // bare cursor value too in case that varies by resolution or plan.
    nextUrl = (typeof raw.next === 'string' && raw.next.startsWith('http'))
      ? raw.next
      : (Number.isFinite(Number(raw.next))
        ? `https://api.openweathermap.org/data/4.0/onecall/timeline/${resolution}?lat=${lat}&lon=${lng}&cnt=${wantCount}&start=${Number(raw.next)}&units=imperial&appid=${apiKey}`
        : null);
  }
...
export async function getForecast(lat, lng, { force = false, debug = false } = {}) {
  ...
  if (!force) {
    ...
    // or a payload cached before the pagination fix with a short hourly
    // array) would otherwise keep serving bad/incomplete data for the
    // full TTL window with no way to self-correct short of someone
    // remembering to pass force=1. Treating a payload with no usable
    // current temp, or fewer than 24 hourly records, as a miss means a
    // code fix here takes effect on the next request, period.
    const usable = cached?.payload?.current?.tempF != null && (cached?.payload?.hourly?.length || 0) >= 24;
  ...
  const payload = {
    ...
    current: nowRecord && {
      ...
      humidity: nowRecord.humidity != null ? Math.round(nowRecord.humidity) : null,
