...
// Creates or updates a timed (non-all-day) event — used for bid
// walks/inspections, which happen at a specific time, unlike a job's
// scheduled date range which is pushed as an all-day block above.
export async function upsertMicrosoftTimedEvent(accessToken, existingEventId, { title, description, startISO, endISO }) {
  const body = {
    subject: title,
    body: { contentType: 'text', content: description || '' },
    isAllDay: false,
