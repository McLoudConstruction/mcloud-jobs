...
const DEFAULTS = {
  ...
  brand_color: '#9b773d',
  header_text_color: null, // null = use the default hardcoded header text color
  color_bg: '#dbd8bf',
  ...
  dashboard_widgets: {},
  proposal_followup_count: 3,
  proposal_followup_interval_days: 4,
};
...
export function SettingsProvider({ children }) {
  ...
  const load = useCallback(async () => {
    ...
    // Postgres returns unset columns as null, and null still wins over a
    // spread default (only undefined is skipped) — so an app_settings row
    // that's never had a field explicitly saved silently clobbers its
    // DEFAULTS value instead of falling through to it. Strip nulls first so
    // an unset column actually uses the default.
    const cleanedData = Object.fromEntries(Object.entries(data || {}).filter(([, v]) => v !== null));
    const merged = { ...DEFAULTS, ...cleanedData };
    setSettings(merged);
    applyTheme(merged);
    setLoading(false);
  }, []);
