...
import { enqueue, getPendingQueue, getAllQueueItems, updateQueueItem, removeQueueItem } from './offlineDb';
...
async function syncOne(item) {
  // This whole function must never throw — whatever happens to this one
  // item, it always resolves with an { ok } result so a Promise.all/
  // allSettled over a batch can't get derailed by it, and so this item
  // always ends up somewhere checkable (removed, 'pending' for retry, or
  // 'failed' with a reason) rather than stuck at 'syncing' forever. The
  // syncStatus='syncing' write used to sit outside this try/catch — if
  // *that* write itself ever threw (an IndexedDB hiccup, which iOS Safari
  // can produce under the same storage/memory pressure documented in
  // imageCompress.js), this function rejected before doing anything else,
  // which under Promise.all() could abort collection of the whole batch's
  // results, and left this specific item's record exactly as it was before
  // this call — 'pending' if the write never landed, but with no guarantee
  // either way. Wrapping it in means every path through this function ends
  // the same, controlled way.
  try {
    await updateQueueItem(item.id, { syncStatus: 'syncing' });
  } catch (err) {
    // Couldn't even mark it syncing — leave it alone (still 'pending', so
    // the next flush wave picks it back up) rather than risk it being
    // half-written.
...
// Turns a settled Promise.allSettled() entry into the same { id, ok, error }
// shape syncOne() normally resolves with — belt-and-suspenders alongside
// syncOne() now never throwing: if something still manages to reject (a bug
// in a future edit, some other unforeseen throw), this stops it from being
// silently dropped out of the results/remaining accounting the way a bare
// Promise.all rejection would.
function settledToResult(item, settled) {
  if (settled.status === 'fulfilled') return settled.value;
  return { id: item.id, ok: false, error: settled.reason?.message || String(settled.reason) };
}
...
async function runFlush() {
  ...
  // Reclaim items stranded at 'syncing'. flushChain (below) guarantees only
  // one flush wave runs at a time in this page's lifetime, so any item
  // still marked 'syncing' when a NEW wave starts here can't be one this
  // same wave is about to touch — it's a leftover from a wave that started
  // and never got to finish at all. In practice that almost always means
  // the tab died mid-upload with zero chance to update its own status
  // (iOS Safari's silent memory-pressure reload is the textbook case —
  // see imageCompress.js). getPendingQueue() deliberately excludes
  // 'syncing' items so a live upload never gets double-synced, but that
  // same exclusion is exactly what made a stranded item invisible to every
  // check, including the direct stuck-item query in InternalUpdatesPanel:
  // not 'pending' (never retried), not 'failed' (no error surfaced), just
  // silently absent. Resetting these to 'pending' before reading the queue
  // means a stranded photo gets swept back up on the very next sync
  // instead of being lost for good.
  const all = await getAllQueueItems();
  const stranded = all.filter(i => i.syncStatus === 'syncing');
  if (stranded.length > 0) {
    await Promise.all(stranded.map(i => updateQueueItem(i.id, { syncStatus: 'pending' })));
  }
  ...
  // allSettled rather than all: one item rejecting (syncOne is now
  // guarded against this, but this is the second layer of defense) must
  // never abort collection of the rest of the batch's results, and must
  // never skip the "remaining" recount below.
  const otherSettled = await Promise.allSettled(otherItems.map(syncOne));
  const otherResults = otherSettled.map((s, i) => settledToResult(otherItems[i], s));
  const photoSettled = await Promise.allSettled(photoItems.map(syncOne));
  const photoResults = photoSettled.map((s, i) => settledToResult(photoItems[i], s));
