...
async function syncOne(item) {
  ...
  try {
    if (item.table === 'job_photos') {
      ...
      // A "Completion" internal update is significant enough to surface on
      // the dashboard notifications feed, not just sit in the job's own
      // update log. This fires at sync time (same as the insert above), so
      // it fires exactly once whether the update was posted online or
      // queued offline and synced later.
      if (item.table === 'job_updates' && item.payload.is_internal && item.payload.category === 'Completion') {
        const { data: job } = await supabase.from('jobs').select('job_number, customer_name').eq('id', item.jobId).maybeSingle();
        ...
        await supabase.from('notifications').insert({
          message: `${jobLabel}${who} marked Completion on an internal update${note}.`,
          job_id: item.jobId,
        });
      }
    }
