...
export async function queueInternalUpdate({ id, jobId, text, category, createdByEmail }) {
  ...
  const item = await enqueue({
    ...
    payload: { id: updateId, job_id: jobId, issues_notes: text || null, category: category || null, created_by_email: createdByEmail, is_internal: true },
  });
  flushQueue();
  return item;
}
