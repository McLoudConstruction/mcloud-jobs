...
//
// Bug fixed here: runFlush() snapshots the pending queue with a single
// getPendingQueue() call at the top. If a second flushQueue() call came
// in while the first's runFlush() was still in flight (e.g. an Internal
// Update posts its note, kicks off a flush, and — before that flush's
// network round trip finishes — the photo loop enqueues a photo and
// calls flushQueue() too), the old code just returned the *first*
// flush's promise and did nothing else. That photo was never in the
// snapshot the in-flight flush read, so it sat as 'pending' until the
// next 30s poll in useOfflineSync — or indefinitely if the person
// checked the Job Photos tab / RFP popup before that timer fired,
// making a correctly-categorized photo look like its folder was never
// created. flushRequestedAgain tracks that a write arrived mid-flush and
// immediately re-runs once the current pass finishes, instead of
