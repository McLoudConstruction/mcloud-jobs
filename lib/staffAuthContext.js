...
const StaffAuthContext = createContext({ session: null, role: null, status: null, fullName: null, loading: true });
...
// need the caller's staff role. fullName is exposed here too so any
// generated document (change orders, work orders, etc.) can autofill the
// "contractor side" signer name from whoever is actually logged in,
...
export function StaffAuthProvider({ children }) {
  ...
  const loadStaffRow = useCallback(async (sess) => {
    ...
    const { data, error } = await supabase.from('staff_users').select('role, status, full_name').eq('id', sess.user.id).maybeSingle();
  ...
  return (
    <StaffAuthContext.Provider value={{ session, role, status, fullName, loading }}>
      {children}
