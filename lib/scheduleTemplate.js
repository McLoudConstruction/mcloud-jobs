...
//   preferredStartDay - default day-of-week this stage type should snap
//                to rather than starting mid-week (e.g. punch list
//                walks). Just the starting default — a person can set
//                or clear this per-job on any individual phase.
export const PHASE_STAGES = [
  { key: 'demo', label: 'Demolition & site prep', trades: ['Demo/Site Prep'], dependsOn: [] },

  // Structural trio: used to be one bundled "Framing & structural" stage
  // covering Framing/Concrete-Foundation/Masonry together as a single
  // phase/bar — which meant a job with no masonry still carried a
  // masonry-labeled bar tangled up with framing, with no way to drop just
  // that trade without losing the others too. Split the same way the
  // rough-in/finish trios already are below: each trade gets its own
  // stage (so it can be individually included, edited, or removed), all
  // depending only on demo, so they still land in the same window and
  // render as concurrent bars on the Gantt whenever more than one is
  // actually present on a job.
  { key: 'framing', label: 'Framing', trades: ['Framing'], dependsOn: ['demo'], parallelWith: ['concrete_foundation', 'masonry'] },
  { key: 'concrete_foundation', label: 'Concrete & Foundation', trades: ['Concrete/Foundation'], dependsOn: ['demo'], parallelWith: ['framing', 'masonry'] },
  { key: 'masonry', label: 'Masonry', trades: ['Masonry'], dependsOn: ['demo'], parallelWith: ['framing', 'concrete_foundation'] },
  { key: 'exterior', label: 'Exterior work', trades: ['Roofing', 'Siding', 'Windows & Doors'], dependsOn: ['framing', 'concrete_foundation', 'masonry'], parallelWith: ['electrical_rough_in', 'plumbing_rough_in', 'hvac_rough_in'] },

  // Rough-in trio: independent of each other, each just needs the
  // structural trades done first. No trade blocks another here — they're
  // separate crews that can run in whatever order or overlap fits the
  // schedule.
  { key: 'electrical_rough_in', label: 'Electrical rough-in', trades: ['Electrical'], dependsOn: ['framing', 'concrete_foundation', 'masonry'] },
  { key: 'plumbing_rough_in', label: 'Plumbing rough-in', trades: ['Plumbing'], dependsOn: ['framing', 'concrete_foundation', 'masonry'] },
  { key: 'hvac_rough_in', label: 'HVAC rough-in', trades: ['HVAC'], dependsOn: ['framing', 'concrete_foundation', 'masonry'] },
  // Inspection now has its own color in the trade palette (it's not a
  // billable trade, so it isn't in SERVICES_OFFERED, but it's a real,
  // colorable phase type) — trades: ['Inspection'] here is purely a
  // coloring hook, it doesn't make this stage a cost item.
  { key: 'rough_in_inspection', label: 'Rough-in inspection', trades: ['Inspection'], dependsOn: ['electrical_rough_in', 'plumbing_rough_in', 'hvac_rough_in'], synthetic: true },
  ...
  // rough-in. All four just need countertops in, no order between them.
  ...
  { key: 'finish_carpentry', label: 'Finish carpentry', trades: ['Finish Carpentry'], dependsOn: ['countertops'] },

  { key: 'landscaping', label: 'Landscaping', trades: ['Landscaping'], dependsOn: ['exterior'] },

  // Punch list and the client's final walkthrough now get their own
  // colors and their own bars — they used to be one combined synthetic
  // stage ("Punch list & walkthrough"), which meant a walkthrough could
  // never be scheduled/tracked as its own event separate from the punch
  // list work itself.
  { key: 'punch_list', label: 'Punch list', trades: ['Punchlist'], dependsOn: ['electrical_finish', 'plumbing_finish', 'tile_finish', 'finish_carpentry', 'painting'], synthetic: true, preferredStartDay: 'monday' },
  { key: 'walkthrough', label: 'Final walkthrough', trades: ['Walkthrough'], dependsOn: ['punch_list'], synthetic: true, preferredStartDay: 'monday' },
];
